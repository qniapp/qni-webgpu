...
test('playwright web server helpers can target an externally managed web server', () => {
  const env = {
  ...
  assert.deepEqual({
    ...
    externalConfig: getPlaywrightWebServerConfig({ env }),
    defaultConfig: getPlaywrightWebServerConfig(),
  }, {
    baseUrl: 'http://127.0.0.1:5999',
    externalConfig: undefined,
    defaultConfig: getWebServerConfig(),
  })
})

test('shared web server config can target an externally managed web server for cucumber support', () => {
  const env = {
    [PLAYWRIGHT_EXTERNAL_SERVER_ENV]: '1',
    [PLAYWRIGHT_BASE_URL_ENV]: 'http://127.0.0.1:5999',
  }
