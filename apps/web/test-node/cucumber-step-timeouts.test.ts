...
test('standard browser startup steps declare explicit cucumber timeouts', async () => {
  ...
  assert.deepEqual({
    givenHasTimeout: /Given\(\s*'the web app is open in the standard WebGPU browser',\s*\{\s*timeout:\s*\w+\s*\}/.test(source),
...
test('plain chromium startup steps declare explicit cucumber timeouts', async () => {
  ...
  assert.deepEqual({
    givenHasTimeout: /Given\(\s*'the web app is open in plain chromium',\s*\{\s*timeout:\s*\w+\s*\}/.test(source),
