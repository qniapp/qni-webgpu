...
#[test]
fn symmetry_breaking_seed_uses_quirk_json() {
    let library = CircuitLibrary::seed();
    let entry = library
        .entries
        .iter()
        .find(|entry| entry.id.as_str() == "symmetry-breaking");
...
#[test]
fn superdense_coding_seed_uses_json() {
    let library = CircuitLibrary::seed();
    let entry = library
        .entries
        .iter()
        .find(|entry| entry.id.as_str() == "superdense-coding");
