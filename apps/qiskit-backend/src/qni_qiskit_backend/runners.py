...
def add_display_saves(
    ...
    for column_index, column in enumerate(request.columns):
        ...
        for probability in probability_by_column.get(column_index, []):
            ...
            qc.save_probabilities(probability_save_qargs(probability, request.qubits), label=label)
...
def controlled_bloch_vector(
    request: BlochOutputRequest, matrix: Sequence[Sequence[complex]], qubits: int
) -> list[float]:
...
def probability_display_response(
    request: ProbabilityOutputRequest, probabilities: Sequence[float], qubits: int
) -> dict[str, Any]:
...
def bloch_save_qargs(request: BlochOutputRequest, qubits: int) -> list[int]:
    bits = [qubits - 1 - request.wire] + bloch_control_bits(request, qubits)
    return [qubits - 1 - bit for bit in bits]


def bloch_control_bits(request: BlochOutputRequest, qubits: int) -> list[int]:
...
def probability_save_qargs(request: ProbabilityOutputRequest, qubits: int) -> list[int]:
...
def probability_control_bits(request: ProbabilityOutputRequest, qubits: int) -> list[int]:
