...
- `mock`: fixed histogram response and fixed ground-state Amplitude display payloads. No quantum simulation; useful for UI/API smoke tests.
...
All runners return bounded histogram output and optional Amplitude display payloads only. The API accepts 1–32 qubits, rejects full statevector / full probability requests, and currently limits exact Amplitude display extraction to <=16 qubits. The web state-vector panel, when refreshed for <=16-qubit GPU-mode runs, is recomputed locally in WebGPU after a successful run rather than transferred from this backend.
...
curl -s http://127.0.0.1:4184/run \
  ...
  -d '{"runner":"mock","qubits":2,"columns":[["H",1],["Amps1",1]],"shots":128,"outputs":{"histogram":true,"amplitudes":[{"gate_id":2,"column":1,"span":1,"base_bit":1,"control_mask":0,"control_value":0,"phase_lock_enabled":true}]}}'
```

Response shape:

```json
{"status":"completed","runner":"mock","qubits":2,"shots":128,"histogram":{"00":128},"truncated":false,"amplitudes":[{"gate_id":2,"span":1,"ket":[[1.0,0.0],[0.0,0.0]],"incoherent":[1.0,0.0],"quality":1.0,"phase_lock_index":0}]}
```
