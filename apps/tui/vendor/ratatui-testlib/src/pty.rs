...
pub fn new(width: u16, height: u16) -> Result<Self> {
    ...
    #[cfg(unix)]
    {
        ...
        if let Some(fd) = pty_pair.master.as_raw_fd() {
            // Ensure PTY reads are non-blocking so update_state can poll safely.
            let flags = fcntl(fd, FcntlArg::F_GETFL)
                .map_err(|e| TermTestError::Pty(format!("failed to get PTY flags: {}", e)))?;
            let flags = OFlag::from_bits_truncate(flags) | OFlag::O_NONBLOCK;
            fcntl(fd, FcntlArg::F_SETFL(flags))
                .map_err(|e| TermTestError::Pty(format!("failed to set PTY non-blocking: {}", e)))?;
        }
    }
    ...
    pub fn write(&mut self, data: &[u8]) -> Result<usize> {
        ...
        let writer = self
            .writer
            .as_mut()
            .ok_or_else(|| TermTestError::Pty("PTY writer unavailable".to_string()))?;
