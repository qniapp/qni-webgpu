...
既定の `mock` runner は量子計算をせず固定 histogram と固定 Amplitude 表示結果を返す。
...
Web UI の `Run GPU` は histogram と表示ブロック単位の Amplitude 結果だけを要求する。Amplitude 結果がない 16量子ビット以下の GPU-mode run では、成功後に状態ベクトルパネルをローカル WebGPU で1回だけ更新する。backend から全状態ベクトルは返さない。
