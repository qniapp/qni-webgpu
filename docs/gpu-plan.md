...
実装メモ:
- ゲートラベルは GPU 側で UV/配置を計算するように変更済み。
- 状態ベクトルは GPU 側で円インスタンスを生成する。
