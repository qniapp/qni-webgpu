# egui-web Cucumber `.feature.md` 導入 Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `apps/egui-web` に `.feature.md` + `@cucumber/cucumber` + Playwright bridge を導入し、最初の 3 scenario を BDD 化しつつ legacy Playwright suite を並走維持する。

**Architecture:** 初回導入は hybrid 構成に限定する。`apps/egui-web/test-support/browser-launch.cjs` と `apps/egui-web/test-support/web-server.cjs` を shared source of truth として追加し、legacy Playwright と新規 Cucumber 経路の両方が同じ browser launch / server lifecycle を使う。BDD 側は `features/*.feature.md` + `features/support/*.cjs` + `features/step_definitions/*.steps.cjs` で構成し、既存 `egui-web.spec.js` の helper は runner 非依存 helper として段階的に抽出する。
...
- Create: `apps/egui-web/test-support/browser-launch.cjs`
  - executable path 解決
  - flagged WebGPU browser 用 launch policy
  - plain chromium error-path 用 launch policy
- Create: `apps/egui-web/test-support/web-server.cjs`
  - `trunk serve --address 127.0.0.1 --port 4174 --no-autoreload` の shared config
  - URL / timeout / reuse policy
- Create: `apps/egui-web/cucumber.cjs`
  - `.feature.md` 実行設定
- Create: `apps/egui-web/features/startup-success.feature.md`
  - `egui webgpu canvas renders content` の 1 scenario のみ
- Create: `apps/egui-web/features/plain-chromium-error.feature.md`
  - `default chromium shows a visible WebGPU error instead of a blank page` の 1 scenario のみ
- Create: `apps/egui-web/features/drag-preview-z-order.feature.md`
  - `dragged palette gate stays above the state panel overlay` の 1 scenario のみ
- Create: `apps/egui-web/features/support/world.cjs`
  - Cucumber World state
- Create: `apps/egui-web/features/support/hooks.cjs`
  - browser/context/page lifecycle
  - screenshot / console / pageerror evidence
- Create: `apps/egui-web/features/support/browser.cjs`
  - shared browser-launch policy adapter
- Create: `apps/egui-web/features/support/server.cjs`
  - shared web-server adapter
- Create: `apps/egui-web/features/support/egui-helpers.cjs`
  - Playwright helper 抽出先
- Create (optional only if needed by implementation): `apps/egui-web/features/support/assertions.cjs`
  - custom assertion helper
- Create: `apps/egui-web/test-node/browser-launch.test.cjs`
  - shared browser launch policy test
- Create: `apps/egui-web/test-node/web-server.test.cjs`
  - shared server policy test
- Create: `apps/egui-web/test-node/cucumber-config.test.cjs`
  - cucumber config / scenario-scope guard test
- Modify: `apps/egui-web/package.json`
- Modify: `apps/egui-web/pnpm-lock.yaml`
  - `@cucumber/cucumber` 追加
  - `test:bdd`, `test:pw-legacy` script 追加
  - `test` は初回導入では切り替えない
- Modify: `apps/egui-web/playwright-browser.cjs`
  - shared launcher へ責務移譲、または shared module に委譲する thin wrapper 化
- Modify: `apps/egui-web/playwright.config.cjs`
  - shared `browser-launch.cjs` / `web-server.cjs` を利用
- Modify: `apps/egui-web/test-node/playwright-browser.test.cjs`
  - shared policy 前提に更新
- Modify: `apps/egui-web/test-node/playwright-config.test.cjs`
  - shared webServer policy を検証する形に更新
- Modify: `apps/egui-web/tests/egui-web.spec.js`
  - helper を最小限共通化する場合のみ
  - 初回導入で scenario 自体の削除はしない
- Modify: `docs/egui-web.md`
...
- Reference spec: `docs/superpowers/specs/2026-04-20-egui-web-cucumber-feature-md-design.md`
...
- `apps/egui-web` 以外へ Cucumber を広げない。
...
- 初回導入では legacy Playwright suite (`apps/egui-web/tests/egui-web.spec.js`) を削除しない。
...
- 実装中は `apps/egui-web` の既存描画・WebGPU 動作を壊さないこと。確認には flagged Chrome 正本運用を維持する。

### Task 0: isolated worktree を準備する

**Files:**
- Modify: none
- Reference: `docs/superpowers/specs/2026-04-20-egui-web-cucumber-feature-md-design.md`
- Reference: `docs/superpowers/plans/2026-04-20-egui-web-cucumber-feature-md.md`
...
cd /home/yasuhito/Work/qni-webgpu && git branch --list feat/egui-web-cucumber-feature-md
```
Expected:
- `git worktree list` に `egui-web-cucumber-feature-md` がない
...
git -C "$REPO" worktree add "$WORKTREE_ROOT/egui-web-cucumber-feature-md" -b feat/egui-web-cucumber-feature-md "$BASE_BRANCH"
```
Expected:
- worktree directory が作成される
- branch `feat/egui-web-cucumber-feature-md` が作成される
...
```bash
WORKTREE_ROOT="${QNI_WORKTREE_ROOT:-$HOME/.config/superpowers/worktrees/qni-webgpu}"
cd "$WORKTREE_ROOT/egui-web-cucumber-feature-md" && git status --short
```
Expected:
- no output
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && node --test test-node/browser-launch.test.cjs test-node/web-server.test.cjs
```
Expected:
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm run test:preflight
```
Expected:
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm exec playwright test --grep 'egui webgpu canvas renders content|default chromium shows a visible WebGPU error instead of a blank page|dragged palette gate stays above the state panel overlay'
```
Expected:
- 3 tests すべて pass
...
Run:
```bash
git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md add \
  apps/egui-web/test-support/browser-launch.cjs \
  apps/egui-web/test-support/web-server.cjs \
  apps/egui-web/test-node/browser-launch.test.cjs \
  apps/egui-web/test-node/web-server.test.cjs \
  apps/egui-web/playwright-browser.cjs \
  apps/egui-web/playwright.config.cjs \
  apps/egui-web/test-node/playwright-browser.test.cjs \
  apps/egui-web/test-node/playwright-config.test.cjs

git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md commit -m "test: share egui-web browser and server policy"
```
Expected:
...
- Create: `apps/egui-web/features/support/server.cjs`
- Create: `apps/egui-web/features/support/egui-helpers.cjs`
- Create (if needed): `apps/egui-web/features/support/assertions.cjs`
- Create: `apps/egui-web/test-node/cucumber-config.test.cjs`
- Modify: `apps/egui-web/package.json`
- Modify: `apps/egui-web/pnpm-lock.yaml`
- Reference: `apps/egui-web/tests/egui-web.spec.js`
- Reference: `docs/superpowers/specs/2026-04-20-egui-web-cucumber-feature-md-design.md`
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && node --test test-node/cucumber-config.test.cjs
```
Expected:
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm install
```
Expected:
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web
TMPDIR="$(mktemp -d)"
...
```bash
git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md add \
  apps/egui-web/package.json \
  apps/egui-web/pnpm-lock.yaml \
  apps/egui-web/cucumber.cjs \
  apps/egui-web/features/support/world.cjs \
  apps/egui-web/features/support/hooks.cjs \
  apps/egui-web/features/support/browser.cjs \
  apps/egui-web/features/support/server.cjs \
  apps/egui-web/features/support/egui-helpers.cjs \
  apps/egui-web/test-node/cucumber-config.test.cjs

git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md commit -m "test: add egui-web cucumber runner scaffolding"
```
Expected:
...
**Files:**
- Create: `apps/egui-web/features/startup-success.feature.md`
- Create: `apps/egui-web/features/step_definitions/startup-success.steps.cjs`
- Modify: `apps/egui-web/features/support/egui-helpers.cjs`
- Reference: `apps/egui-web/tests/egui-web.spec.js`
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm exec cucumber-js --config cucumber.cjs features/startup-success.feature.md
```
Expected:
...
方針:
- 既存 `egui-web.spec.js` の処理を runner 非依存 helper として移す
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm exec playwright test --grep 'egui webgpu canvas renders content'
```
Expected:
- pass
...
```bash
git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md add \
  apps/egui-web/features/startup-success.feature.md \
  apps/egui-web/features/step_definitions/startup-success.steps.cjs \
  apps/egui-web/features/support/egui-helpers.cjs

git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md commit -m "test: add egui-web startup bdd scenario"
```
Expected:
...
**Files:**
- Create: `apps/egui-web/features/plain-chromium-error.feature.md`
- Create: `apps/egui-web/features/step_definitions/plain-chromium-error.steps.cjs`
- Modify: `apps/egui-web/features/support/browser.cjs`
- Modify: `apps/egui-web/features/support/egui-helpers.cjs`
- Reference: `apps/egui-web/tests/egui-web.spec.js`
...
例:
```md
Feature: egui web plain chromium error

  Scenario: Plain chromium shows a visible error instead of a blank page
    Given the egui web app is open in plain chromium
    When the app finishes initializing
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm exec cucumber-js --config cucumber.cjs features/plain-chromium-error.feature.md
```
Expected:
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm exec playwright test --grep 'default chromium shows a visible WebGPU error instead of a blank page'
```
Expected:
- pass
...
```bash
git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md add \
  apps/egui-web/features/plain-chromium-error.feature.md \
  apps/egui-web/features/step_definitions/plain-chromium-error.steps.cjs \
  apps/egui-web/features/support/browser.cjs \
  apps/egui-web/features/support/egui-helpers.cjs

git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md commit -m "test: add egui-web plain chromium error bdd scenario"
```
Expected:
...
**Files:**
- Create: `apps/egui-web/features/drag-preview-z-order.feature.md`
- Create: `apps/egui-web/features/step_definitions/drag-preview-z-order.steps.cjs`
- Modify: `apps/egui-web/features/support/egui-helpers.cjs`
- Modify: `apps/egui-web/features/support/assertions.cjs` (if needed)
- Reference: `apps/egui-web/tests/egui-web.spec.js`
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm exec cucumber-js --config cucumber.cjs features/drag-preview-z-order.feature.md
```
Expected:
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm exec playwright test --grep 'dragged palette gate stays above the state panel overlay'
```
Expected:
- pass
...
```bash
git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md add \
  apps/egui-web/features/drag-preview-z-order.feature.md \
  apps/egui-web/features/step_definitions/drag-preview-z-order.steps.cjs \
  apps/egui-web/features/support/egui-helpers.cjs \
  apps/egui-web/features/support/assertions.cjs

git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md commit -m "test: add egui-web drag preview bdd scenario"
```
Expected:
...
Run:
```bash
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md && bash -n scripts/check-all.sh
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm run test:preflight
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm run test:bdd
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm run test:pw-legacy
```
Expected:
- 4 コマンドすべて success
...
git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md commit -m "test: wire egui-web bdd rollout"
```
Expected:
...
- Reference: `apps/egui-web/**`
- Reference: `scripts/check-all.sh`
- Reference: `docs/egui-web.md`
...
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm run test:bdd
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && pnpm run test:pw-legacy
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web && env PATH="$HOME/.cargo/bin:$PATH" trunk build
cd ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md && git diff --check
```
Expected:
- 5 コマンドすべて success
...
root = Path('/home/yasuhito/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md/apps/egui-web')
...
Run:
```bash
git -C ~/.config/superpowers/worktrees/qni-webgpu/egui-web-cucumber-feature-md status --short
```
Expected:
...
- `apps/egui-web` に `.feature.md` + `@cucumber/cucumber` の実行入口が追加されている
- `apps/egui-web/pnpm-lock.yaml` が依存追加を反映している
...
- `docs/egui-web.md` が新しい test path を説明している
