# egui-web `app.rs` 分割（第4パス）実装計画

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `apps/egui-web/src/lib.rs` から `QniApp` とその app-specific internal types / input / update loop を `apps/egui-web/src/app.rs` に純粋抽出し、挙動を変えずに `lib.rs` をさらに縮小する。
...
- Create: `apps/egui-web/src/app.rs`
  - `PlacedGate`
  - `DragState`
  - `QniApp`
  - `impl QniApp`
...
- Verify: `apps/egui-web/tests/egui-web.spec.js`
  - この pass では変更しない
- Reference spec: `docs/superpowers/specs/2026-04-19-egui-web-app-rs-refactor-design.md`
...
- Modify: `apps/egui-web/src/render.rs`
- Modify: `apps/egui-web/src/layout.rs`
- Reference: `docs/superpowers/specs/2026-04-19-egui-web-app-rs-refactor-design.md`
...
cd /home/yasuhito/Work/qni-webgpu/apps/egui-web && pnpm exec playwright test --grep 'egui webgpu canvas renders content|dragging does not grow state vector until drop|placed circuit gate keeps its visual while dragging another gate|default chromium shows a visible WebGPU error instead of a blank page'
```
Expected:
...
`apps/egui-web/src/app.rs` を作成し、`apps/egui-web/src/lib.rs` に `mod app;` を追加したうえで、次を **同じ編集で原子的に** 移す。
- `PlacedGate`
- `DragState`
- `QniApp`
- `impl QniApp`
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu/apps/egui-web && pnpm exec playwright test --grep 'dragging does not grow state vector until drop|dragged x gate keeps the same visual as after drop|placed circuit gate keeps its visual while dragging another gate|H on q0 and q1 yields uniform superposition'
```
Expected:
- 一致したテストがすべて pass
...
Run:
```bash
git add /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/app.rs /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/lib.rs /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/render.rs /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/layout.rs
git commit -m "refactor: extract egui-web app module"
```
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu && rg -n 'struct PlacedGate|struct DragState|struct QniApp|impl QniApp|impl eframe::App for QniApp' apps/egui-web/src/lib.rs apps/egui-web/src/app.rs
```
Expected:
...
checks = {
    'apps/egui-web/src/render.rs': {'need_qniapp': True, 'need_placedgate': True},
    'apps/egui-web/src/layout.rs': {'need_qniapp': False, 'need_placedgate': True},
}
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu && rg -n 'enum GateKind|struct GateMatrix|struct GateParams|fn gate_matrix|fn gate_params\(|fn gate_params_controlled|struct Colors|pub async fn start|pub async fn read_state_vector' apps/egui-web/src/lib.rs apps/egui-web/src/app.rs
```
Expected:
...
text = Path('apps/egui-web/src/app.rs').read_text().splitlines()
allowed = {
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu && git show --format= --unified=0 HEAD -- apps/egui-web/src/render.rs | rg '^[+-].*pub\(crate\)|^[+-].*pub\(super\)'
```
Expected:
- no output
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu && wc -l apps/egui-web/src/lib.rs apps/egui-web/src/app.rs apps/egui-web/src/render.rs apps/egui-web/src/gpu.rs apps/egui-web/src/layout.rs apps/egui-web/src/icons.rs
```
Expected:
