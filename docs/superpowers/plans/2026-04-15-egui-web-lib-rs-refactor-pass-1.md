# egui-web lib.rs refactor (pass 1) Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extract the obvious low-risk seams from `apps/egui-web/src/lib.rs` into `layout.rs` and `icons.rs` without changing egui-web behavior.

**Architecture:** This pass is a behavior-preserving code-motion refactor only. Move layout/snap helpers into `apps/egui-web/src/layout.rs` and gate drawing helpers into `apps/egui-web/src/icons.rs`, leaving `QniApp`, the update loop, and GPU/shader code in `lib.rs` for now so the runtime glue stays stable.
...
- Create: `apps/egui-web/src/layout.rs`
...
- Create: `apps/egui-web/src/icons.rs`
...
- Reference spec: `docs/superpowers/specs/2026-04-15-egui-web-lib-rs-refactor-design.md`
...
- Create: `apps/egui-web/src/layout.rs`
- Modify: `apps/egui-web/src/lib.rs`
- Reference: `docs/superpowers/specs/2026-04-15-egui-web-lib-rs-refactor-design.md`

- [ ] **Step 1: Run the smallest baseline checks before moving code**

Run:
```bash
cd /home/yasuhito/Work/qni-webgpu/apps/egui-web && cargo check
```
Expected: `Finished`/`Checking` completes successfully before code motion.

- [ ] **Step 2: Add the new module shell and declaration**

Create `apps/egui-web/src/layout.rs` and add `mod layout;` near the top of `apps/egui-web/src/lib.rs`.
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu/apps/egui-web && pnpm exec playwright test --grep 'dragging does not grow state vector until drop|CNOT with control on q1 yields bell state|Control does not affect gates in other columns'
```
...
Run:
```bash
git add /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/lib.rs /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/layout.rs
git commit -m "refactor: extract egui-web layout helpers"
```
...
Create `apps/egui-web/src/icons.rs` and add `mod icons;` near the top of `apps/egui-web/src/lib.rs`.
...
Run:
```bash
git add /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/lib.rs /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/icons.rs
git commit -m "refactor: extract egui-web gate drawing helpers"
```
...
- [ ] **Step 1: Run the pass-1 scoped egui-web verification**
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu && wc -l apps/egui-web/src/lib.rs apps/egui-web/src/layout.rs apps/egui-web/src/icons.rs
```
