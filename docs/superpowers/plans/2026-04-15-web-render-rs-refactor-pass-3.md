# web `render.rs` 分割（第3パス）実装計画

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `apps/web/src/lib.rs` から circuit / palette / state panel の描画責務を `apps/web/src/render.rs` に純粋抽出し、挙動を変えずに `lib.rs` をさらに縮小する。
...
- Verify: `apps/web/tests/web.spec.js`
  - この pass では変更しない
- Reference spec: `docs/superpowers/specs/2026-04-15-web-render-rs-refactor-design.md`
...
- Create: `apps/web/src/render.rs`
- Modify: `apps/web/src/lib.rs`
- Reference: `docs/superpowers/specs/2026-04-15-web-render-rs-refactor-design.md`
...
cd /home/yasuhito/Work/qni-webgpu/apps/web && pnpm exec playwright test --grep 'web canvas renders content|palette panel keeps its corners and shadow while dragging|default chromium shows a visible WebGPU error instead of a blank page'
```
Expected:
...
`apps/web/src/render.rs` を作成し、`apps/web/src/lib.rs` の先頭付近に `mod render;` を追加する。
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu/apps/web && pnpm exec playwright test --grep 'web canvas renders content|palette panel keeps its corners and shadow while dragging'
```
Expected:
- 一致したテストがすべて pass
...
Run:
```bash
git add /home/yasuhito/Work/qni-webgpu/apps/web/src/lib.rs /home/yasuhito/Work/qni-webgpu/apps/web/src/render.rs
git commit -m "refactor: extract web render types"
```
...
```bash
git add /home/yasuhito/Work/qni-webgpu/apps/web/src/lib.rs /home/yasuhito/Work/qni-webgpu/apps/web/src/render.rs
git commit -m "refactor: extract web render methods"
```
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu/apps/web && pnpm exec playwright test --grep 'H on q0 and q1 yields uniform superposition|CNOT with control on q1 yields bell state|Control does not affect gates in other columns|default chromium shows a visible WebGPU error instead of a blank page'
```
Expected:
- 一致したテストがすべて pass
...
```bash
git add /home/yasuhito/Work/qni-webgpu/apps/web/src/lib.rs /home/yasuhito/Work/qni-webgpu/apps/web/src/render.rs
git commit -m "refactor: extract web state panel rendering"
```
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu && rg -n 'fn handle_input\b|fn schedule_drag_repaint\b|impl eframe::App for QniApp|struct Colors\b|pub async fn start|pub async fn read_state_vector|fn gate_params\b|fn gate_params_controlled\b|fn gate_matrix\b' apps/web/src/lib.rs apps/web/src/render.rs
```
Expected:
