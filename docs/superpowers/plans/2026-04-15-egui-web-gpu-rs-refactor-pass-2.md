# egui-web `gpu.rs` 分割（第2パス）実装計画

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `apps/egui-web/src/lib.rs` から GPU / shader / readback の責務を `apps/egui-web/src/gpu.rs` に純粋抽出し、挙動を変えずに `lib.rs` をさらに縮小する。
...
- Create: `apps/egui-web/src/gpu.rs`
  - `StateInstance`
  - `RenderParams`
  - `RenderColors`
...
- Verify: `apps/egui-web/tests/egui-web.spec.js`
  - この pass では変更しない
- Reference spec: `docs/superpowers/specs/2026-04-15-egui-web-gpu-rs-refactor-design.md`
...
- Create: `apps/egui-web/src/gpu.rs`
- Modify: `apps/egui-web/src/lib.rs`
- Reference: `docs/superpowers/specs/2026-04-15-egui-web-gpu-rs-refactor-design.md`
...
cd /home/yasuhito/Work/qni-webgpu/apps/egui-web && pnpm exec playwright test --grep 'egui webgpu canvas renders content|H on q0 and q1 yields uniform superposition|CNOT with control on q1 yields bell state|Control does not affect gates in other columns'
```
Expected:
...
`apps/egui-web/src/gpu.rs` を作成し、`apps/egui-web/src/lib.rs` の先頭付近に `mod gpu;` を追加する。
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu/apps/egui-web && pnpm exec playwright test --grep 'egui webgpu canvas renders content|H on q0 and q1 yields uniform superposition'
```
Expected:
- 一致したテストがすべて pass
...
Run:
```bash
git add /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/lib.rs /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/gpu.rs
git commit -m "refactor: extract egui-web gpu resources"
```
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu/apps/egui-web && pnpm exec playwright test --grep 'H on q0 and q1 yields uniform superposition|CNOT with control on q1 yields bell state|Control does not affect gates in other columns'
```
Expected:
- 一致したテストがすべて pass
...
```bash
git add /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/lib.rs /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/gpu.rs
git commit -m "refactor: extract egui-web gpu callback"
```
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu/apps/egui-web && pnpm exec playwright test --grep 'H on q0 and q1 yields uniform superposition|CNOT with control on q1 yields bell state|default chromium shows a visible WebGPU error instead of a blank page'
```
Expected:
- 一致したテストがすべて pass
...
```bash
git add /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/lib.rs /home/yasuhito/Work/qni-webgpu/apps/egui-web/src/gpu.rs
git commit -m "refactor: extract egui-web gpu readback"
```
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu && rg -n '#\[wasm_bindgen\]|pub async fn start|pub async fn read_state_vector|read_state_vector_impl' apps/egui-web/src/lib.rs apps/egui-web/src/gpu.rs
```
Expected:
...
Run:
```bash
cd /home/yasuhito/Work/qni-webgpu && rg -n 'crate::' apps/egui-web/src/gpu.rs
```
Expected:
- `crate::Colors`
- `crate::GateParams`
