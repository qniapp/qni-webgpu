# web `shared.rs` 抽出 Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `apps/web/src/lib.rs` から shared helper を `apps/web/src/shared.rs` に純粋抽出し、挙動を変えずに crate root をさらに thin-entry 化する。
...
- Modify: `apps/web/src/render.rs`
- Modify: `apps/web/src/colors.rs`
- Reference: `docs/superpowers/specs/2026-04-19-web-shared-rs-refactor-design.md`
...
from pathlib import Path
import re
text = Path('apps/web/src/shared.rs').read_text()
required = [
