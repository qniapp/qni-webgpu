# egui-web Cucumber `.feature.md` 導入設計

## 背景
- `apps/egui-web` の Web テストは現在 `@playwright/test` による **Playwright 直書き** で運用している。
- 実行入口は次の 3 箇所に揃っている。
  - `apps/egui-web/package.json` の `"test": "playwright test"`
  - `scripts/check-all.sh` の `pnpm -C "$ROOT_DIR/apps/egui-web" exec playwright test`
...
## 目的
- `apps/egui-web` の Web テストに **`.feature.md` + `@cucumber/cucumber`** を導入し、ユーザー視点のシナリオ記述を Markdown with Gherkin へ移す。
...
## 非目的
- 既存 `apps/egui-web/tests/egui-web.spec.js` を一度に全面削除すること。
- `@playwright/test` 依存 helper を一気に全面書き換えること。
- ブラウザ選択ロジックや WebGPU フラグ方針を変更すること。
- `apps/egui-web` 以外のテスト体系（`apps/tui`, `apps/mcp-qni` など）へ Cucumber を広げること。
...
- 既存ブラウザ解決ロジック: `apps/egui-web/playwright-browser.cjs` を再利用
...
- `apps/egui-web/features/step_definitions/*.steps.cjs`
...
- 初期導入では `apps/egui-web/tests/egui-web.spec.js` をすぐ削除しない。
- まずは Cucumber 側に **代表 3 シナリオだけ移植**し、その後に全面移行判断を行う。
- helper の共通化に合わせて、将来的に `egui-web.spec.js` からロジックを段階的に切り出す。
...
方針:
- `pnpm -C apps/egui-web run test:bdd` は ad hoc に server を起動しない。
...
1. `egui webgpu canvas renders content`
...
## 既存 helper の整理方針
現在の `egui-web.spec.js` には、Cucumber にそのまま持ち込みにくい `@playwright/test` runner 寄り helper がある。
...
- `apps/egui-web/tests/egui-web.spec.js` は初回導入では削除しない。
- CI の入口を即日全面切り替えしない。
- 既存の `test-node` preflight は保持する。
- plain chromium failure scenario と flagged Chrome success scenario の両方を維持する。
- 変更は `apps/egui-web` と必要最小限の root script / CI 配線に留める。

## 受け入れ条件
- `apps/egui-web` に `@cucumber/cucumber` ベースの実行入口が追加されている。
...
- `playwright.config.cjs` が `apps/egui-web/test-support/browser-launch.cjs` と `apps/egui-web/test-support/web-server.cjs` を参照している。
...
- 必要なら `pnpm -C apps/egui-web run test:pw-legacy`
...
- node-level test で `apps/egui-web/test-support/browser-launch.cjs` の主要 launch policy を検証すること
- node-level test で `apps/egui-web/test-support/web-server.cjs` の `command` / `url` / `timeout` / `reuseExistingServer` を検証すること
