# web `lib.rs` thin-entry 化設計（次パス）

## 背景
- `apps/web/src/lib.rs` は pass 1（`layout.rs` / `icons.rs`）、pass 2（`gpu.rs`）、pass 3（`render.rs`）、pass 4（`app.rs`）の抽出後、367 LOC まで縮小した。
...
## 実装手順
1. `apps/web/src/gates.rs` と `apps/web/src/colors.rs` を追加し、`lib.rs` に `mod gates; mod colors;` を宣言する。
...
## 受け入れ条件
- `apps/web/src/gates.rs` と `apps/web/src/colors.rs` が追加されている。
