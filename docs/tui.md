...
- q0 と q1 は常に保持される。
- 量子ビット数は最大 16 まで。
- 状態ベクトルは表示領域の最下段に円で表示される（量子ビット数に応じて 2^n 個）。ただし q0 しか使われていない場合は |0>, |1> の2個だけ表示する。表示順は |00>, |01>, |10>, |11> ... の順（q0 が最下位ビット）。
...
- CTRL は小さな四角として表示し、現状はドラッグ中も追加の枠線は付けない。
...
- 未確定・確定ラインがない場合は、開始位置（初期状態 `|00>`）に確定ラインを表示する。
