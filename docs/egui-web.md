...
- 状態ベクトル計算は GPU の `STATE_COMPUTE_SHADER` (per-gate dispatch + ping-pong) で実行する。CPU 側にミラーシミュレータ (`bloch::simulate`) を残してあるが、Step 3 (Measurement の GPU 化) のあとで完全に削除予定。Measurement のある回路だけ `cpu_state_override` で CPU 結果を直接アップロードする。
- Bloch ベクトル計算は GPU の `BLOCH_REDUCE_SHADER` (workgroup_size=64 で 1 workgroup の reduction) が処理する。`linearize_ops` が列ごとに `ApplyGate` と `CaptureBloch` を交互に並べ、`gpu.rs::prepare` がそれを順番に dispatch する。出力は `bloch_output_buffer` に shader-side で書き込み、新しい staging buffer (per-readback) に copy → `map_async` で非同期 readback。`BLOCH_CACHE` (sequence number 付き) に出揃った値が貯まり、`app.rs::update` が次フレーム冒頭でドレインして `self.bloch_vectors` を上書き + `request_repaint` する。
- `read_bloch_vectors()` (wasm_bindgen) で同期的にスナップショットを取得できる。Playwright からは `window.__eguiReadBlochVectors()` 経由で `[gateId, x, y, z, …]` の `Float32Array` として読める (`features/support/egui-helpers.ts:readBlochVectors` を経由)。
