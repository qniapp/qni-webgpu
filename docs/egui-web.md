...
TS 化済み support module は `features/support/assertions.ts`、`features/support/browser.ts`、`features/support/hooks.ts`、`features/support/server.ts`、`features/support/world.ts` で、残りの CJS support modules は `require(...) as ...` で薄く型を付ける。
