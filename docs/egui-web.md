...
- 状態ベクトル計算は GPU の `STATE_COMPUTE_SHADER` (per-gate dispatch + ping-pong) で実行する。CPU 側にミラーシミュレータ (`bloch::simulate`) を残してあるが、これは Bloch ベクトルの集計と Measurement の collapse を CPU でやっている期間の暫定 fallback で、最終的に Step 2/3 で GPU 化したら撤去する。Measurement のある回路だけ `cpu_state_override` で CPU 結果を直接アップロードする。
