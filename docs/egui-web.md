...
- Circuit picker の並び替えは `docs/circuit-picker-reorder-mock.html` に従う。行全体を mouse drag handle とし、kebab は submenu trigger のまま。drag ghost は Y 軸だけ追従し、2px の Flexoki blue-600 drop indicator で挿入位置を示す。submenu の Move up / Move down は 1 段ずつ動かす既存操作として残す。
