/* ─────────────────────────────────────────────────────────────────────
 * s-dagger-gate.js — <s-dagger-gate> カスタム要素
 *
 * 仕様: docs/design-system/s-dagger-gate.html (canonical 定義)
 *
 * 他のデザインシステムページから S† を参照するときは、inline SVG ではなく、
 * 本要素を使って canonical な見た目をそのまま埋め込む。
 *
 * 使い方:
 *   <script src="components/s-dagger-gate.js"></script>
 *
 *   <s-dagger-gate></s-dagger-gate>                <!-- 既定 (rest) -->
 *   <s-dagger-gate state="hover"></s-dagger-gate>  <!-- ホバー枠 + 背景の空きを強制 -->
 *   <s-dagger-gate state="drag"></s-dagger-gate>   <!-- drag preview (本体 purple-600) -->
 *
 * 設計判断:
 * - Shadow DOM (open): ホスト側の CSS が `.s-dagger-gate` などを再定義しても見た目が壊れない
 * - SVG path は実装 (apps/web/assets/icons/sdagger.svg) と一致する Geist 700 outline
...
<div class="body">
  <!-- S† の glyph。path d は apps/web/assets/icons/sdagger.svg と完全一致。 -->
...
`;
  ...
  class SDaggerGate extends HTMLElement {
    constructor() {
      super();
  ...
  customElements.define('s-dagger-gate', SDaggerGate);
})();
