/* ─────────────────────────────────────────────────────────────────────
 * t-gate.js — <t-gate> カスタム要素
 *
 * 仕様: docs/design-system/t-gate.html (canonical 定義)
 *
 * 他のデザインシステムページから T を参照するときは、inline SVG ではなく、
 * 本要素を使って canonical な見た目をそのまま埋め込む。
 *
 * 使い方:
 *   <script src="components/t-gate.js"></script>
 *
 *   <t-gate></t-gate>                <!-- 既定 (rest) -->
 *   <t-gate state="hover"></t-gate>  <!-- ホバー枠 + 背景の空きを強制 -->
 *   <t-gate state="drag"></t-gate>   <!-- drag preview (本体 purple-600) -->
 *
 * 設計判断:
 * - Shadow DOM (open): ホスト側の CSS が `.t-gate` などを再定義しても見た目が壊れない
 * - SVG path は実装 (apps/web/assets/icons/t.svg) と一致する Geist 700 outline
...
<div class="body">
  <!-- T の glyph。path d は apps/web/assets/icons/t.svg と完全一致。 -->
...
`;
  ...
  class TGate extends HTMLElement {
    constructor() {
      super();
  ...
  customElements.define('t-gate', TGate);
})();
