/* ─────────────────────────────────────────────────────────────────────
 * rx-gate.js — <rx-gate> カスタム要素
 *
 * 仕様: docs/design-system/rx-gate.html (canonical 定義)
 *
 * 他のデザインシステムページから RX を参照するときは、inline SVG ではなく、
 * 本要素を使って canonical な見た目をそのまま埋め込む。
 *
 * 使い方:
 *   <script src="components/rx-gate.js"></script>
 *
 *   <rx-gate></rx-gate>                <!-- 既定 (rest) -->
 *   <rx-gate state="hover"></rx-gate>  <!-- ホバー枠 + 背景の空きを強制 -->
 *   <rx-gate state="drag"></rx-gate>   <!-- drag preview (本体 purple-600) -->
 *
 * 設計判断:
 * - Shadow DOM (open): ホスト側の CSS が `.rx-gate` などを再定義しても見た目が壊れない
 * - SVG path は実装 (apps/web/assets/icons/rx.svg) と一致する Geist 700 outline
...
<div class="body">
  <!-- RX の glyph。path d は apps/web/assets/icons/rx.svg と完全一致。 -->
  <svg viewBox="0 0 48 48" aria-hidden="true">
<g transform="translate(8.710 31.838) scale(0.022080 -0.022080)">
  ...
  <g transform="translate(23.724 31.838) scale(0.022080 -0.022080)">
    <path d="M14 0 250 356 16 710H143L318 440L490 710H616L382 358L619 0H492L316 274L140 0Z" fill="currentColor"/>
  </g>
  </svg>
</div>
`;

  class RxGate extends HTMLElement {
    constructor() {
      super();
  ...
  customElements.define('rx-gate', RxGate);
})();
