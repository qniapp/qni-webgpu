...
- パレットと回路で同じ高解像度ラスタ画像を縮小サンプリングするため、サブピクセル位置差で H / X / Y / Z / √X の太さが変わらない。
...
| 画面確認 | `http://127.0.0.1:4180/#%7B%22cols%22%3A%5B%5B%22H%22%5D%2C%5B%22X%22%5D%2C%5B%22Y%22%5D%2C%5B%22Z%22%5D%2C%5B%22X%5E%C2%BD%22%5D%5D%7D` |
...
| C: SVG → PNG 焼き付け | `scripts/extract-gate-svg.py` が SVG と PNG を生成。`build.rs` が PNG アルファを RLE 化し、初回描画時に egui テクスチャ化 | Playwright 実測でパレット / 回路の H, X, Y, Z, √X が一致（内側の白画素: H 44 px、X 36 px、Y 34 px、Z 35 px、√X 62 px） | 8,105,856 bytes（-2,633） | SVG 生成後に PNG も生成されるため追加実装ほぼなし | 維持できる。SVG が正、PNG は派生物 | `rsvg-convert` または `magick` が再生成時だけ必要。通常ビルドは Rust の `png` ビルド時依存だけ | 採用 |
...
- `scripts/extract-gate-svg.py`
  - Geist から `h.svg`, `x.svg`, `y.svg`, `z.svg`, `plus.svg`, `sqrtx.svg` を生成。
...
| `QNI_EGUI_WEB_EXTERNAL_SERVER=1 QNI_EGUI_WEB_BASE_URL=http://127.0.0.1:4180 pnpm -C apps/egui-web exec playwright test tests/egui-web-palette-visual.spec.ts --grep "PNG raster H, X, Y, Z and √X" --workers=1` | 通過 |
